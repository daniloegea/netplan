...
/* convenience macro to put the offset of a NetplanNetDefinition field into
 * "void* data" */
...
/* convenience macro to avoid strdup'ing a string into a field if it's already
 * set. */
#define set_str_if_null(dst, src)                                                                                      \
    {                                                                                                                  \
        if (dst) {                                                                                                     \
 ...
 *
 * Returns: TRUE on success, FALSE if the document is malformed; @error gets set
 * then.
 */
static gboolean
...
#define assert_type(ctx, n, t)                                                                                         \
    {                                                                                                                  \
        if (!assert_type_fn(ctx, n, t, error))                                                                         \
            return FALSE;                                                                                              \
    }
...
typedef gboolean (*custom_map_handler)(NetplanParser* npp, yaml_node_t* node, const char* prefix, const void* data,
                                       GError** error);
 ...
 */
static gboolean
process_mapping(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const mapping_entry_handler* handlers,
                GList** out_values, GError** error)
{
...
/*
 * Handler for setting a MAC address field from a scalar node, inside a given
 * struct
 * @entryptr: pointer to the beginning of the to-be-modified data structure
 * @data: offset into entryptr struct where the const char* field to write is
 *        located
...
handle_generic_mac(NetplanParser* npp, yaml_node_t* node, void* entryptr, const void* data, GError** error)
{
    ...
    if (regexec(&re, scalar(node), 0, NULL, 0) != 0)
        return yaml_error(npp, node, error,
                          "Invalid MAC address '%s', must be XX:XX:XX:XX:XX:XX or "
 ...
 * @entryptr: pointer to the beginning of the to-be-modified data structure
 * @data: offset into entryptr struct where the boolean field to write is
 * located
 */
static gboolean
handle_generic_bool(NetplanParser* npp, yaml_node_t* node, void* entryptr, const void* data, GError** error)
{
    ...
    if (g_ascii_strcasecmp(scalar(node), "true") == 0 || g_ascii_strcasecmp(scalar(node), "on") == 0
        || g_ascii_strcasecmp(scalar(node), "yes") == 0 || g_ascii_strcasecmp(scalar(node), "y") == 0)
        v = TRUE;
    else if (g_ascii_strcasecmp(scalar(node), "false") == 0 || g_ascii_strcasecmp(scalar(node), "off") == 0
             || g_ascii_strcasecmp(scalar(node), "no") == 0 || g_ascii_strcasecmp(scalar(node), "n") == 0)
        v = FALSE;
    else
...
/*
 * Handler for setting a HashTable field from a mapping node, inside a given
 * struct
 * @entryptr: pointer to the beginning of the to-be-modified data structure
 * @data: offset into entryptr struct where the boolean field to write is
 * located
 ...
 */
static gboolean
handle_generic_map(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, void* entryptr, const void* data,
                   GError** error)
{
    guint offset     = GPOINTER_TO_UINT(data);
 ...
 */
static gboolean
handle_generic_datalist(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, void* entryptr, const void* data,
                        GError** error)
{
    guint offset = GPOINTER_TO_UINT(data);
...
/**
 * Generic handler for setting a npp->current.netdef string field from a scalar
 * node
 * @data: offset into NetplanNetDefinition where the const char* field to write
 * is located
 */
static gboolean
...
/**
 * Generic handler for setting a npp->current.netdef ID/iface name field from a
 * scalar node
 * @data: offset into NetplanNetDefinition where the const char* field to write
 * is located
 */
...
handle_embedded_switch_mode(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    if (g_strcmp0(scalar(node), "switchdev") != 0 && g_strcmp0(scalar(node), "legacy") != 0)
        return yaml_error(npp, node, error,
                          "Value of 'embedded-switch-mode' needs to be "
                          "'switchdev' or 'legacy'");
...
/**
 * Generic handler for setting a npp->current.netdef ID/iface name field
 * referring to an existing ID from a scalar node. This handler also includes a
 * special case handler for OVS VLANs, switching the backend implicitly to OVS
 * for such interfaces
 * @data: offset into NetplanNetDefinition where the NetplanNetDefinition* field
 * to write is located
 */
static gboolean
    ...
    if (!ref) {
        ...
        *dest                        = ref;
        ...
        if (netdef->type == NETPLAN_DEF_TYPE_VLAN && ref->backend == NETPLAN_BACKEND_OVS) {
            g_debug("%s: VLAN defined for openvswitch interface, choosing OVS "
                    "backend",
                    netdef->id);
...
/**
 * Generic handler for setting a npp->current.netdef MAC address field from a
 * scalar node
 * @data: offset into NetplanNetDefinition where the const char* field to write
 * is located
 */
...
/**
 * Generic handler for setting a npp->current.netdef gboolean field from a
 * scalar node
 * @data: offset into NetplanNetDefinition where the gboolean field to write is
 * located
 */
static gboolean
...
/**
 * Generic handler for tri-state settings that can bei "UNSET", "TRUE", or
 * "FALSE".
 * @data: offset into NetplanNetDefinition where the guint field to write is
 * located
 */
static gboolean
...
/**
 * Generic handler for setting a npp->current.netdef guint field from a scalar
 * node
 * @data: offset into NetplanNetDefinition where the guint field to write is
 * located
 */
...
handle_netdef_ip4(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    /* FIXME: stop excluding this from coverage; refactor address handling
     * instead */
    // LCOV_EXCL_START
    if (prefix_len)
        return yaml_error(npp, node, error,
                          "invalid address: a single IPv4 address (without "
...
handle_netdef_ip6(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    if (prefix_len)
        return yaml_error(npp, node, error,
                          "invalid address: a single IPv6 address (without "
                          "/prefixlength) is required");
...
handle_match_driver(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* _, GError** error)
{
    ...
    /* We overload the 'driver' setting for matches; such that it can either be
     * a single scalar specifying a single driver glob/match, or a sequence of
     * many globs any of which must match. */
    if (node->type == YAML_SCALAR_NODE) {
        ...
        return yaml_error(npp, node, error,
                          "invalid type for 'driver': must be a scalar or a "
                          "sequence of scalars");
...
static gboolean
handle_access_point_datalist(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* data,
                             GError** error)
{
...
static const mapping_entry_handler nm_backend_settings_handlers[] = {
    ...
    /* Fallback mode, to support all NM settings of the NetworkManager netplan
       backend */
    {"passthrough",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_netdef_datalist}},
     netdef_offset(backend_settings.nm.passthrough)},
    {NULL}};
...
static const mapping_entry_handler ap_nm_backend_settings_handlers[] = {
    ...
    {"stable-id",
     YAML_SCALAR_NODE,
     {.generic = handle_access_point_str},
     access_point_offset(backend_settings.nm.stable_id)},
    {"device", YAML_SCALAR_NODE, {.generic = handle_access_point_str}, access_point_offset(backend_settings.nm.device)},
    /* Fallback mode, to support all NM settings of the NetworkManager netplan
       backend */
    {"passthrough",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_access_point_datalist}},
     access_point_offset(backend_settings.nm.passthrough)},
    {NULL}};
...
NETPLAN_ABI struct NetplanWifiWowlanType NETPLAN_WIFI_WOWLAN_TYPES[] = {
...
handle_wowlan(NetplanParser* npp, yaml_node_t* node, const void* _, GError** error)
{
    ...
    if (npp->current.netdef->wowlan > NETPLAN_WIFI_WOWLAN_DEFAULT
        && npp->current.netdef->wowlan & NETPLAN_WIFI_WOWLAN_TYPES[0].flag)
...
{
    if (g_ascii_strcasecmp(scalar(node), "0") != 0 && g_ascii_strcasecmp(scalar(node), "forever") != 0) {
...
/*
 * Handler for setting an array of IP addresses from a sequence node, inside a
 * given struct
 ...
 */
static gboolean
handle_generic_addresses(NetplanParser* npp, yaml_node_t* node, gboolean check_zero_prefix, GArray** ip4, GArray** ip6,
                         GError** error)
{
    g_assert(ip4);
    g_assert(ip6);
        ...
        *prefix_len = '\0';
        ...
        if (value) {
            if (!is_ip4_address(addr) && !is_ip6_address(addr))
                return yaml_error(npp, node, error,
                                  "malformed address '%s', must be X.X.X.X/NN "
                                  "or X:X:X:X:X:X:X:X/NN",
                                  scalar(entry));
            ...
            for (unsigned i = 0; i < npp->current.netdef->address_options->len; ++i) {
                NetplanAddressOptions* opts =
                    g_array_index(npp->current.netdef->address_options, NetplanAddressOptions*, i);
                /* check for multi-pass parsing, return early if options for
        ...
        if (is_ip4_address(addr)) {
            ...
            /* Do not append the same IP (on multiple passes), if it is already
             * contained */
        ...
        return yaml_error(npp, node, error, "malformed address '%s', must be X.X.X.X/NN or X:X:X:X:X:X:X:X/NN",
                          scalar(entry));
    }
...
{
    return handle_generic_addresses(npp, node, TRUE, &(npp->current.netdef->ip4_addresses),
                                    &(npp->current.netdef->ip6_addresses), error);
}
...
static gboolean
handle_wifi_access_points(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* data,
                          GError** error)
{
    for (yaml_node_pair_t* entry = node->data.mapping.pairs.start; entry < node->data.mapping.pairs.top; entry++) {
        ...
        if (npp->current.netdef->access_points
            ...
            ret = yaml_error(npp, key, error, "%s: Duplicate access point SSID '%s'", npp->current.netdef->id,
                             access_point->ssid);
        }
...
/**
 * Handler for bridge "interfaces:" list. We don't store that list in
 * npp->current.netdef, but set npp->current.netdef's ID in all listed
 * interfaces' "bond" or "bridge" field.
 * @data: ignored
 */
static gboolean
    ...
    for (yaml_node_item_t* i = node->data.sequence.items.start; i < node->data.sequence.items.top; i++) {
        ...
        if (!component) {
            ...
            if (component->backend == NETPLAN_BACKEND_OVS) {
                g_debug("%s: Bridge contains openvswitch interface, choosing "
                        "OVS backend",
...
{
    if (!(strcmp(scalar(node), "balance-rr") == 0 || strcmp(scalar(node), "active-backup") == 0
          || strcmp(scalar(node), "balance-xor") == 0 || strcmp(scalar(node), "broadcast") == 0
          || strcmp(scalar(node), "802.3ad") == 0 || strcmp(scalar(node), "balance-tlb") == 0
          || strcmp(scalar(node), "balance-alb") == 0 || strcmp(scalar(node), "balance-tcp") == 0
          ||                                         // only supported for OVS
    ...
    if (!strcmp(scalar(node), "balance-tcp") || !strcmp(scalar(node), "balance-slb")) {
        g_debug("%s: mode '%s' only supported with openvswitch, choosing this "
                "backend",
...
handle_bond_interfaces(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    for (yaml_node_item_t* i = node->data.sequence.items.start; i < node->data.sequence.items.top; i++) {
        ...
        if (!component) {
            ...
            if (component->backend == NETPLAN_BACKEND_OVS) {
                g_debug("%s: Bond contains openvswitch interface, choosing OVS "
                        "backend",
                        npp->current.netdef->id);
                npp->current.netdef->backend = NETPLAN_BACKEND_OVS;
            }
...
handle_nameservers_addresses(NetplanParser* npp, yaml_node_t* node, const void* _, GError** error)
{
    for (yaml_node_item_t* i = node->data.sequence.items.start; i < node->data.sequence.items.top; i++) {
        ...
        return yaml_error(npp, node, error, "malformed address '%s', must be X.X.X.X or X:X:X:X:X:X:X:X",
                          scalar(entry));
    }
...
NETPLAN_ABI struct NetplanOptionalAddressType NETPLAN_OPTIONAL_ADDRESS_TYPES[] = {
    {"ipv4-ll", NETPLAN_OPTIONAL_IPV4_LL}, {"ipv6-ra", NETPLAN_OPTIONAL_IPV6_RA}, {"dhcp4", NETPLAN_OPTIONAL_DHCP4},
    {"dhcp6", NETPLAN_OPTIONAL_DHCP6},     {"static", NETPLAN_OPTIONAL_STATIC},   {NULL},
};
...
handle_vxlan_flags(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    for (yaml_node_item_t* i = node->data.sequence.items.start; i < node->data.sequence.items.top; i++) {
        ...
        if (!found) {
            return yaml_error(npp, node, error, "invalid value for %s: '%s'", scalar(key_node), scalar(entry));
        }
    }
    return TRUE;
}
...
/* TODO: (cyphermox) Refactor the functions below. There's a lot of room for
 * reuse. */
...
handle_routes_scope(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    if (g_ascii_strcasecmp(route->scope, "global") == 0 || g_ascii_strcasecmp(route->scope, "link") == 0
...
handle_routes_type(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    if (g_ascii_strcasecmp(route->type, "unicast") == 0 || g_ascii_strcasecmp(route->type, "anycast") == 0
        || g_ascii_strcasecmp(route->type, "blackhole") == 0 || g_ascii_strcasecmp(route->type, "broadcast") == 0
        || g_ascii_strcasecmp(route->type, "local") == 0 || g_ascii_strcasecmp(route->type, "multicast") == 0
        || g_ascii_strcasecmp(route->type, "nat") == 0 || g_ascii_strcasecmp(route->type, "prohibit") == 0
        || g_ascii_strcasecmp(route->type, "throw") == 0 || g_ascii_strcasecmp(route->type, "unreachable") == 0
...
static gboolean
handle_bridge_port_priority(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* data,
                            GError** error)
{
    for (yaml_node_pair_t* entry = node->data.mapping.pairs.start; entry < node->data.mapping.pairs.top; entry++) {
        yaml_node_t *key, *value;
        ...
        if (!component) {
            ...
            if (*endptr != '\0' || v > 63)
                return yaml_error(npp, node, error,
                                  "invalid port priority value (must be "
                                  "between 0 and 63): %s",
                                  scalar(value));
...
static const mapping_entry_handler bridge_params_handlers[] = {
    ...
    {"path-cost",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_bridge_path_cost}},
     netdef_offset(bridge_params.path_cost)},
    {"port-priority",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_bridge_port_priority}},
     netdef_offset(bridge_params.port_priority)},
...
static const mapping_entry_handler routes_handlers[] = {
    ...
    {"advertised-receive-window",
     YAML_SCALAR_NODE,
     {.generic = handle_routes_guint},
     route_offset(advertised_receive_window)},
    {NULL}};
...
handle_routes(NetplanParser* npp, yaml_node_t* node, const void* _, GError** error)
{
    ...
    for (yaml_node_item_t* i = node->data.sequence.items.start; i < node->data.sequence.items.top; i++) {
        ...
        if (!route->scope) {
            if (g_ascii_strcasecmp(route->type, "local") == 0 || g_ascii_strcasecmp(route->type, "nat") == 0)
        ...
        if ((g_ascii_strcasecmp(route->scope, "link") == 0 || g_ascii_strcasecmp(route->scope, "host") == 0)
            && !route->to) {
            yaml_error(npp, node, error, "link and host routes must specify a 'to' IP");
            goto err;
        } else if (g_ascii_strcasecmp(route->type, "unicast") == 0 && g_ascii_strcasecmp(route->scope, "global") == 0
                   && (!route->to || !route->via)) {
...
handle_bond_primary_slave(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    if (!component) {
        ...
        /* If this is not the primary pass, the primary slave might already be
         * equally set. */
        if (!g_strcmp0(npp->current.netdef->bond_params.primary_slave, scalar(node))) {
            ...
            return yaml_error(npp, node, error, "%s: bond already has a primary slave: %s", npp->current.netdef->id,
                              npp->current.netdef->bond_params.primary_slave);
...
static const mapping_entry_handler bond_params_handlers[] = {
    ...
    {"fail-over-mac-policy",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_str},
     netdef_offset(bond_params.fail_over_mac_policy)},
    ...
    {"packets-per-slave",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_guint},
     netdef_offset(bond_params.packets_per_slave)},
    {"primary-reselect-policy",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_str},
     netdef_offset(bond_params.primary_reselect_policy)},
    {"resend-igmp", YAML_SCALAR_NODE, {.generic = handle_netdef_guint}, netdef_offset(bond_params.resend_igmp)},
    {"learn-packet-interval",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_str},
     netdef_offset(bond_params.learn_interval)},
...
handle_vxlan_source_port(NetplanParser* npp, yaml_node_t* node, const void* _, GError** error)
{
    ...
    if (node->data.sequence.items.top - node->data.sequence.items.start != 2)
        return yaml_error(npp, node, error, "%s: Expected exactly two values for port-range", npp->current.netdef->id);
...
handle_dhcp_identifier(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    return yaml_error(npp, node, error, "invalid DHCP client identifier type '%s'",
                      npp->current.netdef->dhcp_identifier);
}
...
handle_tunnel_key_mapping(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* _, GError** error)
{
    ...
    /* We overload the 'key[s]' setting for tunnels; such that it can either be
     * a single scalar with the same key to use for both input, output and
     * private keys, or a mapping where one can specify each. */
    if (node->type == YAML_SCALAR_NODE) {
...
static gboolean
handle_wireguard_peer_key_mapping(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* _,
                                  GError** error)
{
...
handle_ovs_bridge_fail_mode(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    if (npp->current.netdef->type != NETPLAN_DEF_TYPE_BRIDGE)
        return yaml_error(npp, node, error,
                          "Key 'fail-mode' is only valid for interface type "
                          "'openvswitch bridge'");
...
{
    const char* supported[] = {"OpenFlow10", "OpenFlow11", "OpenFlow12", "OpenFlow13",
                               "OpenFlow14", "OpenFlow15", "OpenFlow16", NULL};
    unsigned i              = 0;
    guint offset            = GPOINTER_TO_UINT(data);
...
handle_ovs_bridge_protocol(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    if (npp->current.netdef->type != NETPLAN_DEF_TYPE_BRIDGE)
        return yaml_error(npp, node, error,
                          "Key 'protocols' is only valid for interface type "
                          "'openvswitch bridge'");
...
handle_ovs_bridge_controller_connection_mode(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    if (npp->current.netdef->type != NETPLAN_DEF_TYPE_BRIDGE)
        return yaml_error(npp, node, error,
                          "Key 'controller.connection-mode' is only valid for "
                          "interface type 'openvswitch bridge'");

    if (g_strcmp0(scalar(node), "in-band") && g_strcmp0(scalar(node), "out-of-band"))
        return yaml_error(npp, node, error,
                          "Value of 'connection-mode' needs to be 'in-band' or "
                          "'out-of-band'");
...
handle_ovs_bridge_controller_addresses(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    if (npp->current.netdef->type != NETPLAN_DEF_TYPE_BRIDGE)
        return yaml_error(npp, node, error,
                          "Key 'controller.addresses' is only valid for "
                          "interface type 'openvswitch bridge'");
...
static const mapping_entry_handler ovs_controller_handlers[] = {
    {"addresses",
     YAML_SEQUENCE_NODE,
     {.generic = handle_ovs_bridge_controller_addresses},
     netdef_offset(ovs_settings.controller.addresses)},
    {"connection-mode",
     YAML_SCALAR_NODE,
     {.generic = handle_ovs_bridge_controller_connection_mode},
...
static const mapping_entry_handler ovs_backend_settings_handlers[] = {
    {"external-ids",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_netdef_map}},
     netdef_offset(ovs_settings.external_ids)},
    {"other-config",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_netdef_map}},
     netdef_offset(ovs_settings.other_config)},
    ...
    {"mcast-snooping",
     YAML_SCALAR_NODE,
     {.generic = handle_ovs_bridge_bool},
     netdef_offset(ovs_settings.mcast_snooping)},
...
handle_ovs_backend(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* _, GError** error)
{
    ...
    if (npp->current.netdef->type != NETPLAN_DEF_TYPE_BOND && npp->current.netdef->type != NETPLAN_DEF_TYPE_BRIDGE) {
        ...
        /* Non-bond/non-bridge interfaces might still be handled by the networkd
         * backend */
...
#define COMMON_LINK_HANDLERS                                                                                           \
    {"accept-ra", YAML_SCALAR_NODE, {.generic = handle_accept_ra}, netdef_offset(accept_ra)},                          \
        ...
        {"ipv6-address-token",                                                                                         \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_addrtok},                                                                           \
         netdef_offset(ip6_addr_gen_token)},                                                                           \
...
#define COMMON_BACKEND_HANDLERS                                                                                        \
    ...
    {                                                                                                                  \
        "openvswitch", YAML_MAPPING_NODE,                                                                              \
        {                                                                                                              \
            .map = {.custom = handle_ovs_backend }                                                                     \
        }                                                                                                              \
    }
...
#define PHYSICAL_LINK_HANDLERS                                                                                         \
    {"match", YAML_MAPPING_NODE, {.map = {.custom = handle_match}}},                                                   \
        ...
        {"receive-checksum-offload",                                                                                   \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(receive_checksum_offload)},                                                                     \
        {"transmit-checksum-offload",                                                                                  \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(transmit_checksum_offload)},                                                                    \
        {"tcp-segmentation-offload",                                                                                   \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(tcp_segmentation_offload)},                                                                     \
        {"tcp6-segmentation-offload",                                                                                  \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(tcp6_segmentation_offload)},                                                                    \
        {"generic-segmentation-offload",                                                                               \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(generic_segmentation_offload)},                                                                 \
        {"generic-receive-offload",                                                                                    \
         YAML_SCALAR_NODE,                                                                                             \
         {.generic = handle_netdef_tristate},                                                                          \
         netdef_offset(generic_receive_offload)},                                                                      \
    {                                                                                                                  \
        "large-receive-offload", YAML_SCALAR_NODE, {.generic = handle_netdef_tristate},                                \
            netdef_offset(large_receive_offload)                                                                       \
    }
...
static const mapping_entry_handler ethernet_def_handlers[] = {
    ...
    {"virtual-function-count",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_guint},
     netdef_offset(sriov_explicit_vf_count)},
    {"embedded-switch-mode",
     YAML_SCALAR_NODE,
     {.generic = handle_embedded_switch_mode},
     netdef_offset(embedded_switch_mode)},
    {"delay-virtual-functions-rebind",
     YAML_SCALAR_NODE,
     {.generic = handle_netdef_bool},
     netdef_offset(sriov_delay_virtual_functions_rebind)},
...
static gboolean
handle_network_ovs_settings_global(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* data,
                                   GError** error)
{
...
handle_network_ovs_settings_global_ports(NetplanParser* npp, yaml_node_t* node, const void* data, GError** error)
{
    ...
    for (yaml_node_item_t* iter = node->data.sequence.items.start; iter < node->data.sequence.items.top; iter++) {
        ...
        if (item + 2 != pair->data.sequence.items.top) {
            return yaml_error(npp, pair, error,
                              "An openvswitch peer port sequence must have "
                              "exactly two entries");
        }
...
handle_network_type(NetplanParser* npp, yaml_node_t* node, const char* key_prefix, const void* data, GError** error)
{
    for (yaml_node_pair_t* entry = node->data.mapping.pairs.start; entry < node->data.mapping.pairs.top; entry++) {
        ...
        if (key_prefix && npp->null_fields) {
            full_key = g_strdup_printf("%s\t%s", key_prefix, key->data.scalar.value);
            if (g_hash_table_contains(npp->null_fields, full_key)
                || node_is_nulled_out(&npp->doc, value, full_key, npp->null_fields))
                continue;
        }
...
static const mapping_entry_handler ovs_network_settings_handlers[] = {
    {"external-ids",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_network_ovs_settings_global}},
     ovs_settings_offset(external_ids)},
    {"other-config",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_network_ovs_settings_global}},
     ovs_settings_offset(other_config)},
    {"protocols",
     YAML_SEQUENCE_NODE,
     {.generic = handle_network_ovs_settings_global_protocol},
     ovs_settings_offset(protocols)},
...
static const mapping_entry_handler network_handlers[] = {
    ...
    {"ethernets",
     YAML_MAPPING_NODE,
     {.map = {.custom = handle_network_type}},
     GUINT_TO_POINTER(NETPLAN_DEF_TYPE_ETHERNET)},
...
process_document(NetplanParser* npp, GError** error)
{
    ...
    if (g_hash_table_size(npp->missing_id) > 0) {
        ...
        return yaml_error(npp, missing->node, error, "%s: interface '%s' is not defined", missing->netdef_id, key);
    }
...
netplan_parser_load_yaml(NetplanParser* npp, const char* filename, GError** error)
{
    ...
    if (stat(filename, &info) < 0) {
        g_set_error(error, G_FILE_ERROR, errno, "Cannot stat %s: %s", filename, strerror(errno));
        return FALSE;
...
netplan_state_import_parser_results(NetplanState* np_state, NetplanParser* npp, GError** error)
{
    if (npp->parsed_defs) {
        ...
        gpointer key, value;
        char* regdom = NULL;
        g_debug("We have some netdefs, pass them through a final round of "
                "validation");
        ...
        if (!validate_default_route_consistency(npp, npp->parsed_defs, &recoverable)) {
            g_warning("Problem encountered while validating default route "
                      "consistency."
                      "Please set up multiple routing tables and use "
                      "`routing-policy` instead.\n"
                      "Error: %s",
                      (recoverable) ? recoverable->message : "");
        ...
        while (g_hash_table_iter_next(&iter, &key, &value)) {
            g_assert(np_state->netdefs == NULL || g_hash_table_lookup(np_state->netdefs, key) == NULL);
            ...
            if (nd->regulatory_domain) {
                if (!regdom)
                    ...
                    g_warning("%s: Conflicting regulatory-domain (%s vs %s)", nd->id, regdom, nd->regulatory_domain);
            }
    ...
    /* We need to reset those fields manually as we transfered ownership of the
       underlying data to out. If we don't do this, netplan_clear_parser will
       deallocate data that we don't own anymore. */
    npp->ordered = NULL;
...
netplan_parser_reset(NetplanParser* npp)
{
    ...
    /* These pointers are non-owning, it's not our place to free their
     * resources*/
...
extract_null_fields(yaml_document_t* doc, yaml_node_t* node, GHashTable* null_fields, char* key_prefix)
{
    ...
    switch (node->type) {
        ...
        // LCOV_EXCL_STOP
        case YAML_SCALAR_NODE:
            if (g_ascii_strcasecmp("null", scalar(node)) == 0 || g_strcmp0((char*) node->tag, YAML_NULL_TAG) == 0
