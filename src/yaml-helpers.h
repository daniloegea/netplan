...
#define YAML_MAPPING_OPEN(event_ptr, emitter_ptr)                                                                      \
    {                                                                                                                  \
        yaml_mapping_start_event_initialize(event_ptr, NULL, (yaml_char_t *) YAML_MAP_TAG, 1,                          \
                                            YAML_BLOCK_MAPPING_STYLE);                                                 \
        if (!yaml_emitter_emit(emitter_ptr, event_ptr))                                                                \
            goto err_path;                                                                                             \
    }
...
#define YAML_SEQUENCE_OPEN(event_ptr, emitter_ptr)                                                                     \
    {                                                                                                                  \
        yaml_sequence_start_event_initialize(event_ptr, NULL, (yaml_char_t *) YAML_SEQ_TAG, 1,                         \
...
#define YAML_SCALAR_PLAIN(event_ptr, emitter_ptr, scalar)                                                              \
    {                                                                                                                  \
        yaml_scalar_event_initialize(event_ptr, NULL, (yaml_char_t *) YAML_STR_TAG, (yaml_char_t *) scalar,            \
                                     strlen(scalar), 1, 0, YAML_PLAIN_SCALAR_STYLE);                                   \
        if (!yaml_emitter_emit(emitter_ptr, event_ptr))                                                                \
            goto err_path;                                                                                             \
    }
...
#define YAML_NULL_PLAIN(event_ptr, emitter_ptr)                                                                        \
    yaml_scalar_event_initialize(event_ptr, NULL, (yaml_char_t *) YAML_NULL_TAG, (yaml_char_t *) "null",               \
                                 strlen("null"), 1, 0, YAML_PLAIN_SCALAR_STYLE);                                       \
    if (!yaml_emitter_emit(emitter_ptr, event_ptr))                                                                    \
        goto err_path;
...
#define YAML_SCALAR_QUOTED(event_ptr, emitter_ptr, scalar)                                                             \
    {                                                                                                                  \
        yaml_scalar_event_initialize(event_ptr, NULL, (yaml_char_t *) YAML_STR_TAG, (yaml_char_t *) scalar,            \
                                     strlen(scalar), 1, 1, YAML_DOUBLE_QUOTED_SCALAR_STYLE);                           \
        if (!yaml_emitter_emit(emitter_ptr, event_ptr))                                                                \
            goto err_path;                                                                                             \
    }
